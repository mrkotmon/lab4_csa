...
Ввод-вывод отображён в память (порт ввода `@0x1FF8`, порт вывода `@0x1FFC`); отдельных программно-видимых регистров портов нет.

**Сигналы:**

- Защёлки:
  - `latch_pc` — защёлкнуть выбранное значение в `PC`;
  - `latch_ir` — защёлкнуть выбранное слово в `IR`;
  - `latch_res` — защёлкнуть выход АЛУ (или эффективный адрес) в `RES`;
  - `latch_dr` — защёлкнуть данные памяти в `DR`;
  - `latch_vres` — защёлкнуть результат векторного АЛУ в `VRES`;
  - `latch_spc`, `latch_sfl` — сохранить `PC`/флаги при входе в прерывание;
  - `wr_reg` — запись результата в скалярный регистр `R[WA]`;
  - `wr_vreg` — запись результата в векторный регистр `V[WA]`;
  - `set_ie` / `clr_ie` — установить/сбросить разрешение прерываний (`EI`/`DI`);
  - `set_ih` / `clr_ih` — войти/выйти из обработчика (вход в прерывание / `IRET`).
- Сигналы записи:
  - `write_mem` — запись слова в память данных; если адрес = `0x1FFC`, декодер адреса направляет запись в порт вывода (`write_io`). Чтение порта ввода — это чтение по адресу `0x1FF8`: декодер выдаёт значение порта вместо ячейки памяти.
- Мультиплексоры:
  - `sel_pc` — выбор следующего значения `PC`:
    - `sel_pc_alu` — выход АЛУ (`PC+4`, адрес ветвления `PC+offset`, а также `JMP`/`JR` через АЛУ);
    - `sel_pc_spc` — сохранённый `PC` (`IRET`);
    - `sel_pc_handler` — адрес обработчика из памяти (`MEM[0]`).
  - `sel_addr` — выбор адреса памяти:
    - `sel_addr_pc` — адрес из `PC` (выборка команды);
    - `sel_addr_ea` — эффективный адрес `RES` (`LW`/`SW`/`VLD`/`VST`);
    - `sel_addr_zero` — константа `0` (вектор прерывания).
  - `sel_A` — выбор первого операнда АЛУ:
    - `sel_A_reg` — значение регистра `RD1` (`rs1`);
    - `sel_A_pc` — значение `PC` (для `PC+4` и `PC+offset`).
  - `sel_B` — выбор второго операнда АЛУ:
    - `sel_B_reg` — значение регистра `RD2` (`rs2`);
    - `sel_B_imm` — расширенный знаковый `imm`;
    - `sel_B_four` — константа `4` (для `PC+4`).
  - `sel_wb` — выбор источника записи в скалярный регистр:
    - `sel_wb_alu` — результат АЛУ `RES` (в т.ч. сумма для `VRED`);
    - `sel_wb_mem` — данные из памяти `DR` (`LW`).
  - `sel_wdmem` — выбор данных для записи в память:
    - `sel_wdmem_reg` — значение регистра `RD2` (`SW`);
    - `sel_wdmem_vec` — векторный регистр (`VST`).
  - `sel_vwb` — выбор источника записи в векторный регистр:
    - `sel_vwb_valu` — результат векторного АЛУ (`VADD`/…/`VCMP`);
    - `sel_vwb_mem` — данные из памяти (`VLD`);
    - `sel_vwb_bcast` — скаляр `RD1` (`VBC`).
- АЛУ (скалярное):
  - `add` — сложение;
  - `sub` — вычитание;
  - `mul` — умножение;
  - `div` — целочисленное деление (округление к нулю);
  - `mod` — остаток от деления;
  - `and` — побитовое И;
  - `or` — побитовое ИЛИ;
  - `xor` — побитовое исключающее ИЛИ.
- Векторное АЛУ:
  - `vadd` / `vsub` / `vmul` / `vdiv` — поэлементная арифметика;
  - `vcmp` — поэлементное сравнение (1/0);
  - `vred` — свёртка вектора в скаляр (сумма элементов).

Адреса регистрового файла (`RA1`, `RA2`, `WA`) берутся из полей инструкции (`rs1`, `rs2/rd`, `rd`); для `JMP`/`JR` управляющее устройство подставляет `R0`, используя его нулевое значение как операнд.

Все операции выполняются в 32-битном формате; результат приводится к 32-битному знаковому (обрезание по модулю 2³²).
...
Реализован в классе `Processor`. Управление hardwired (без микрокода): дешифратор формата сводится к операции `opcode >> 5` — старшие 3 бита опкода задают тип инструкции, по которому автомат выставляет нужные сигналы из списка выше.

Конечный автомат стадий: `FETCH → DECODE → EXECUTE → [MEMORY] → [WRITEBACK] → CHECK_IRQ`. Одна стадия = один такт (потактовая точность), поэтому число тактов инструкции равно числу её активных стадий.

- `FETCH` — `sel_addr_pc`, чтение слова из памяти → `latch_ir`, `PC ← PC+4`;
- `DECODE` — декодирование `IR` (поля `rd`/`rs1`/`rs2`/`imm`) и формирование управляющих сигналов; такт тратится для соблюдения потактовой точности;
- `EXECUTE` — выбор операндов (`sel_A`, `sel_B`), операция АЛУ → `latch_res`; для переходов — пересчёт `PC` через `sel_pc`;
- `MEMORY` — `sel_addr_ea`; `LW` → `latch_dr`, `SW` → `write_mem`; `VLD`/`VST` выполняются по одному слову за такт (память однопортовая);
- `WRITEBACK` — запись результата (`wr_reg` через `sel_wb` или `wr_vreg` через `sel_vwb`), обновление флагов `Z`/`N`;
- `CHECK_IRQ` — между инструкциями: при `IE=1`, `IH=0` и наличии запроса прерывания — `latch_spc`/`latch_sfl`, `set_ih`, `sel_pc_handler` (`PC ← MEM[0]`); выход из обработчика по `IRET` (`sel_pc_spc`, восстановление флагов, `clr_ih`). Вложенные прерывания откладываются, пока `IH=1`;
- `HLT` останавливает автомат.
